 ...
 * Per docs/07_CONNECTOR_SPECIFICATION.md §1.2 + docs/05_MONOREPO_STRUCTURE.md.
 ...
 /**
  * Backward-compat shim: the Phase-1 `fetchSnapshot(pair)` is now an alias
  * for `fetchTicker(pair)`. New code should call `fetchTicker` directly.
  */
 export async function fetchSnapshot(
   c: Connector,
   pair: import('@nova-app/shared').TradingPair,
 ): Promise<import('@nova-app/shared').PriceSnapshot | null> {
